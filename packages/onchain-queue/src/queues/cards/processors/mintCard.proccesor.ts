...
@Processor(ONCHAIN_QUEUES.QUEUE_MINT_CARD)
export class MintCardProcessor {
  constructor(
  ...
  logger = new Logger(MintCardProcessor.name);
  @Process({ name: ONCHAIN_JOB.JOB_MINT_CARD, concurrency: 10 })
  async detectEv(job: Job<LogsReturnValues>) {
    ...
    try {
      ...
      this.queue.add(ONCHAIN_JOB.JOB_MINT_CARD, event);
    }
  }
}
