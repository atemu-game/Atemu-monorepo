...
@Processor(ONCHAIN_QUEUES.QUEUE_ADD_POINT)
export class BliztPointProcessor {
  ...
  logger = new Logger(BliztPointProcessor.name);
  @Process({ name: ONCHAIN_JOB.JOB_ADD_POINT, concurrency: 10 })
  async detectAddPoint(job: Job<LogsReturnValues>) {
    ...
    try {
      ...
      this.queue.add(ONCHAIN_JOB.JOB_ADD_POINT, event);
    }
  }
}
